...
タイトル画面には特定の依頼主のストーリーは焼き込まない。プレイヤーは「Stock Crash Co.」という工作会社の一員であり、ステージごとに異なる依頼（今回はバーガー・キングダム側の依頼）を請け負う、というステージ選択式の構成を想定している。各ステージ固有のストーリーはブリーフィング画面の「依頼内容」で説明する（詳細は4章）。
...
> 1. **ブリーフィング：** 目標金額と、自分たちが取引する担当企業（バーガー・キングダムに固定、相手企業は自動的にピザ・パレス）を確認する。  
...
2社は経済性格が非対称になっている（企業選択UIは廃止したが、Company側のパラメータとしては両社とも残しており、将来的な拡張やバランス調整のベースとして使う）。

| 企業 | 基準株価 | 値動きの倍率 | 敵工作員の激しさ | 性格 |
| :---- | :---- | :---- | :---- | :---- |
| バーガー・キングダム（担当企業） | $140 | 緩やか（popularity x1.5 / dirtiness x2.0） | 14〜22秒間隔・汚れ+6 | 安定型 |
| ピザ・パレス（相手企業） | $70 | 激しい（popularity x3.0 / dirtiness x4.0） | 7〜12秒間隔・汚れ+10 | ハイリスク型 |
...
### **4.3 担当企業とロケット砲による勝利条件**

担当企業は選択制ではなく、常に「バーガー・キングダム」に固定されている（相手企業は自動的に「ピザ・パレス」になる）。
